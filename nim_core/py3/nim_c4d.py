...
class nim_fileUI( gui.GeDialog ) :
                    ...
                    if type(self.nim.Dict( elem ))==type(list()) :
                        for listItem in self.nim.Dict( elem ) :
                            ...
                            #  Set ID and Task Folder :
                                ...
                                #  Set IDs :
                                if 'name' in list(listItem.keys()) and listItem['name']==self.nim.name( elem ) :
                                    self.nim.set_ID( elem=elem, ID=listItem['ID'] )
                                    break
                                elif 'basename' in list(listItem.keys()) and listItem['basename']==self.nim.name( elem ) :
                                    self.nim.set_ID( elem=elem, ID=None )
                                    break
                                elif 'showname' in list(listItem.keys()) and listItem['showname']==self.nim.name( elem ) :
                                    self.nim.set_ID( elem=elem, ID=listItem['ID'] )
                                    break
    ...
    def populate_elem( self, elem='' ) :
        ...
        jobList=[]
        if elem=='job' :
            for key in list(self.nim.Dict(elem).keys()) : jobList.append(key)
                ...
                if type(item) is type(dict()) :
                    ...
                    #  Versions :
                        ...
                        if self.mode.lower() not in ['save', 'saveas', 'pub', 'publish'] :
                            ...
                            if 'note' in list(item.keys()) : verItem=item['filename']+' - '+item['note']
                            ...
                            if self.nimPrefs.name( elem )==verItem :
                                ...
                                if 'filepath' in list(item.keys()) and item['filepath'] :
                                    self.SetString( self.nim.inputIDs['verFilepath'], item['filepath'] )
                                if 'username' in list(item.keys()) and item['username'] :
                                    self.SetString( self.nim.inputIDs['verUser'], item['username'] )
                                if 'date' in list(item.keys()) and item['date'] :
                                    self.SetString( self.nim.inputIDs['verDate'], item['date'] )
                                if 'note' in list(item.keys()) and item['note'] :
