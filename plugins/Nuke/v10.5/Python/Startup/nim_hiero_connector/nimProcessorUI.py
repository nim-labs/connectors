...
class NimProcessorUIBase(IProcessorUI):
  ...
  def checkUnrenderedComps(self, exportItems):
    ...
    for compItem in self.findCompItems(exportItems):
      ...
      unrendered, outOfDate = isCompItemMissingRenders(compItem)
      if unrendered:
        unrenderedNkSources.add(source)
      elif outOfDate:
        outOfDateNkSources.add(source)

    continueExport = True
    renderComps = False

    # Show a message box and give the user the option to either render the comps or skip them
    if unrenderedNkSources or outOfDateNkSources:
      ...
      if clickedButton == cancelButton:
        ...
        # If the user selected 'Render', render all unrendered or out of date comps.
        if renderComps:
          compsToRender = unrenderedNkSources | outOfDateNkSources
          self._preset.setCompsToRender(compsToRender)
        # If the user selected 'Skip', exclude unrendered comps completely. Comps
        # which are out of date will still be included in the export
        else:
  ...
  def setTaskContent(self, preset):
    ...
    # than the widgets it created, otherwise it can lead to crashes in PySide.
    self._contentUI = None
