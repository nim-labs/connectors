...
print ("Loading: nim_nukeStudioUI")
...
def saveDialog():
	...
	#Get Current Project
	currentProject = nimProjectHelpers.activeProject()
		...
		try:
			...
			if self.nim_versionChooser:
				if len(self.nim_versions)>0:
					...
					for key, value in sorted(list(self.nim_versionDict.items()), reverse=True):
