...
__revisions__ = \
    """
    1.0.5 - Scott B - Fixed bug with shows and assets not populating
    1.0.4 - Scott B - Added NIM API sys.path
                      Changed OSX default temp folder to ~/.nim/temp
    1.0.3 - Scott B - Split import and export tool into two separate tools
                      ...
                      Jobs ordered by descending name
...
class NIMCinesyncDailiesNotesImporter(QtGui.QMainWindow):
    ...
    def updateJobs(self):
        ...
        for jobName in sorted(jobs, reverse=True):
            self.jobsComboBox.addItem(QtGui.QIcon(':/ui/icons/jobs.png'), jobName, jobs.get(jobName))
    ...
    def updateShows(self):
        ...
        try:
            ...
            if not shows:
                self.log.debug('No NIM Shows found')
                return
...
class nimCinesyncPreferences(QtGui.QDialog):
    def __init__(self, mainWindow, parent=None):
        ...
        if 'win32' in sys.platform:
            ...
            self.OSXTEMPDIR = os.path.join(os.path.expanduser("~"), '.nim', 'temp')
