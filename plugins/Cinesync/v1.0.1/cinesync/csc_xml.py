...
def session_to_xml(session):
    ...
    root = ET.Element('session', xmlns=cinesync.SESSION_V3_NAMESPACE,
                      version=str(cinesync.SESSION_V3_XML_FILE_VERSION), fromScript=str(True), ownerShouldAutomaticallyDownload=str(session.ownerShouldAutomaticallyDownload),
                      guestShouldAutomaticallyDownload=str(session.guestShouldAutomaticallyDownload), sessionFeatures=session.get_session_features())
...
def init_media_common(elem, media_obj):
    ...
    media_obj.frame_rate = float(elem.get('framesPerSecond') or 0.0)
